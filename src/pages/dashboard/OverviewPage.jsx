...
function StatCard({ label, value, sub, icon: Icon, iconClass, borderClass, onClick }) {
  return (
    <button
      ...
      } ${onClick ? 'transition-colors hover:bg-slate-50 cursor-pointer' : 'cursor-default'}`}>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
        ...
        <div className="rounded-xl border border-slate-200 bg-white">
          <div className="border-b border-slate-200 px-5 py-4">
            ...
            <p className="mt-0.5 text-xs text-slate-500">Current state of sales operations</p>
          </div>
          <div className="grid gap-3 p-5 sm:grid-cols-2">
            <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
              <span className="rounded-lg bg-emerald-50 p-2 text-emerald-600"><CheckCircle className="h-4 w-4" /></span>
              <div>
                <p className="text-xs text-slate-500">Approved Quotes Total</p>
                <p className="text-2xl font-bold text-slate-900">{isLoading ? '—' : metrics.approvedTotal}</p>
              </div>
            </div>
            <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
              <span className="rounded-lg bg-amber-50 p-2 text-amber-600"><FileText className="h-4 w-4" /></span>
              <div>
                <p className="text-xs text-slate-500">Draft Quotes Open</p>
                <p className="text-2xl font-bold text-slate-900">{isLoading ? '—' : metrics.draftsOpen}</p>
              </div>
            </div>
            <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900">{isLoading ? '—' : (metrics.requestsOpen ?? metrics.opportunitiesOpen)}</p>
              </div>
            </div>
            <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900">{isLoading ? '—' : formatCentsToDollars(metrics.approvedValueTodayCents)}</p>
              </div>
            </div>
            <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 sm:col-span-2">
              <span className="rounded-lg bg-slate-200 p-2 text-slate-500"><RefreshCw className="h-4 w-4" /></span>
              <div>
                <p className="text-xs text-slate-500">Total Contacts</p>
                <p className="text-2xl font-bold text-slate-900">{isLoading ? '—' : metrics.contactsTotal}</p>
        ...
        <div className="rounded-xl border border-slate-200 bg-white">
          <div className="border-b border-slate-200 px-5 py-4">
            ...
            <p className="mt-0.5 text-xs text-slate-500">Latest workflow notifications</p>
          </div>
          <div className="p-5">
            {isLoading ? (
              ...
              <div className="py-8 text-center">
                <CheckCircle className="mx-auto mb-2 h-7 w-7 text-slate-200" />
